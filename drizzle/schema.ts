...
export const usersTable = pgTable(
  "users",
  {
    ...
    // see the whole vocabulary catalog. Never defaulted and never set at sign-in.
...
// A vocabulary is the namespace that owns a term concept. The default
// MatSci-SAM vocabulary keeps the original /vocabulary/<term> identifiers;
// community vocabularies resolve below /vocabulary/<vocabulary>/<term>.
// Collections remain independent and may reference terms from any vocabulary.
// This small allocation table is the concurrency-safe owner of the shared
// one-segment namespace: a root MatSci-SAM term and a community vocabulary
// must never claim the same /vocabulary/<slug> path. Database triggers keep it
// synchronized with the two owning tables.
export const vocabularyRootRoutesTable = pgTable(
  "vocabularyRootRoutes",
  {
    slug: text().primaryKey(),
    ownerKind: text().notNull()
  },
  (table) => [
    check(
      "vocabularyRootRoutes_slug_shape",
      sql`${table.slug} ~ '^[a-z0-9][a-z0-9_-]*$'`
    ),
    check(
      "vocabularyRootRoutes_owner_kind",
      sql`${table.ownerKind} IN ('default_term', 'default_alias', 'vocabulary')`
    )
  ]
)

export type Vocabulary = typeof vocabulariesTable.$inferSelect
export const vocabulariesTable = pgTable(
  "vocabularies",
  {
    // The slug is both the stable namespace key and the public path segment.
    // It is assigned once and never follows later title changes.
    slug: text().primaryKey(),
    title: text().notNull(),
    description: text(),
    // Exactly one vocabulary owns the legacy root identifiers and receives
    // new terms contributed while the user is working in Everything.
  ...
  (table) => [
    uniqueIndex("vocabularies_one_default_unique")
      .on(table.isDefault)
      .where(sql`${table.isDefault}`),
    check(
      "vocabularies_slug_shape",
      sql`${table.slug} ~ '^[a-z0-9][a-z0-9_-]*$'`
    ),
    check("vocabularies_title_nonblank", sql`btrim(${table.title}) <> ''`)
  ]
)
...
export const termsTable = pgTable(
  "terms",
  {
    ...
    vocabularySlug: text()
      ...
      .notNull(),
    term: text().notNull(),
    // Human-readable public identifier. The slug is unique inside its
    // vocabulary, not across the catalog, so two communities may define the
    // same label as genuinely different concepts.
    slug: text().notNull(),
  ...
  (table) => [
    uniqueIndex("terms_vocabulary_term_unique").on(
      table.vocabularySlug,
      sql`lower(btrim(${table.term}))`
    ),
    uniqueIndex("terms_vocabulary_slug_unique").on(
      table.vocabularySlug,
      table.slug
    ),
    check("terms_term_nonblank", sql`btrim(${table.term}) <> ''`),
    check("terms_slug_shape", sql`${table.slug} ~ '^[a-z0-9][a-z0-9_-]*$'`),
    // These names are static children below a community term route. Keeping
...
export const termsTableRelations = relations(termsTable, ({ one, many }) => ({
  vocabulary: one(vocabulariesTable, {
    fields: [termsTable.vocabularySlug],
    references: [vocabulariesTable.slug]
  }),
  definitions: many(definitionsTable),
...
// A term may move to a different vocabulary namespace, but a published route
// never disappears. Each alias records one former route and points straight
// at the term row, so repeated moves still redirect directly to the current
// canonical route rather than creating redirect chains.
export type TermRouteAlias = typeof termRouteAliasesTable.$inferSelect
export const termRouteAliasesTable = pgTable(
  "termRouteAliases",
  {
    vocabularySlug: text()
      .references(() => vocabulariesTable.slug)
      .notNull(),
    termSlug: text().notNull(),
    termId: integer()
  ...
  (table) => [
    primaryKey({
      columns: [table.vocabularySlug, table.termSlug],
      name: "termRouteAliases_vocabularySlug_termSlug_pk"
    }),
    index("term_route_aliases_term_idx").on(table.termId),
    check(
      "termRouteAliases_term_slug_shape",
      sql`${table.termSlug} ~ '^[a-z0-9][a-z0-9_-]*$'`
    ),
    check(
...
export const termRouteAliasesTableRelations = relations(
  termRouteAliasesTable,
  ({ one }) => ({
    vocabulary: one(vocabulariesTable, {
      fields: [termRouteAliasesTable.vocabularySlug],
      references: [vocabulariesTable.slug]
    }),
    term: one(termsTable, {
      fields: [termRouteAliasesTable.termId],
      references: [termsTable.id]
    }),
    createdBy: one(usersTable, {
      fields: [termRouteAliasesTable.createdById],
      references: [usersTable.id]
    })
  })
)
...
export const aiContributionSuggestionsTable = pgTable(
  "aiContributionSuggestions",
  {
    ...
    requestedById: integer()
      .references(() => usersTable.id)
      .notNull(),
    // The namespace the draft was requested for. Persisting it prevents a
    // community switch in another tab from publishing a new-term draft into
...
// separate worklists: they may contain local terms or references to terms in
// another vocabulary without changing the identity of either concept.
//
...
export const communitiesTable = pgTable(
  "communities",
  {
    ...
    slug: text().notNull().unique(),
    // The community's SKOS concept scheme. Created with the same slug as the
    // community and kept explicit so the namespace remains referentially
    // complete even if other vocabulary kinds are added later.
    vocabularySlug: text()
      .references(() => vocabulariesTable.slug)
      .notNull()
      .unique(),
  ...
  (t) => [
    ...
    // A community slug is its vocabulary namespace. Keeping both columns
    // equal makes the public route stable and rules out the default scheme.
    check(
      "communities_vocabulary_matches_slug",
      sql`${t.vocabularySlug} = ${t.slug} AND ${t.vocabularySlug} <> 'matsci-sam'`
    )
  ]
)

export const vocabulariesTableRelations = relations(
  vocabulariesTable,
  ({ one, many }) => ({
    createdBy: one(usersTable, {
      fields: [vocabulariesTable.createdById],
      references: [usersTable.id]
    }),
    community: one(communitiesTable, {
      fields: [vocabulariesTable.slug],
      references: [communitiesTable.vocabularySlug]
    }),
